                        ...
                        {data.map((repo: Repository, index: number) => {
                            ...
                            return (
                                <div
                                    key={repo.full_name}
                                    onClick={() => {
                                        router.push(`/repo/${repo.name}?branch=${repo.default_branch}&full_name=${repo.full_name}&pub=false`)
                                    }}
                                    className="cursor-pointer rounded-md border px-4 py-3 font-mono text-sm"
                                >
                                    @{username}/{repo.name}
                                </div>
                            );
